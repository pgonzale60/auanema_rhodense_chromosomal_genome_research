...
fexpoTelo <- ggarrange(pExpTelo, clipLenplot, 
                       nrow = 1, labels = c("", "C"))
ggsave("report/figures/telomere_features.jpg",
       fexpoTelo, units = "in", 
       width = 8.5*1.1,
       height = 11*0.6)
...
ggplot(telomil, aes(x = average_coverage,
                              y = telomere_gap_average + 0.5)) +
