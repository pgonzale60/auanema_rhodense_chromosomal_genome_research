...
mask <- r_gr
if(length(t_gr) > 0) t_gr <- GenomicRanges::setdiff(t_gr, mask); mask <- GenomicRanges::union(mask, t_gr)
if(length(s_gr) > 0) s_gr <- GenomicRanges::setdiff(s_gr, mask); mask <- GenomicRanges::union(mask, s_gr)
...
rna_all <- bind_rows(rna_list) %>% 
  ...
  mutate(chr = sub("SUPER_", "Chr ", as.character(Chrom)),
...
priority_info <- olap_df %>%
  ...
  mutate(grs_tr_total = sum(family_span)) %>%
  filter(family_span / grs_tr_total > 0.05) %>%
  ungroup()

# Categorize GRS and families for color intercalation
grs_order <- grs_bed %>% group_by(Chrom) %>% mutate(grs_type = case_when(row_number() == 2 ~ "central", TRUE ~ "terminal")) %>% ungroup()
...
central_fams <- priority_info %>% filter(grs_type == "central") %>% pull(Family_ID) %>% unique()
terminal_fams <- priority_info %>% filter(grs_type == "terminal") %>% pull(Family_ID) %>% unique()
...
span_rank <- olap_df %>% group_by(Family_ID) %>% summarise(total = sum(Length))
central_only <- span_rank %>% filter(Family_ID %in% central_only) %>% arrange(desc(total)) %>% pull(Family_ID)
terminal_only <- span_rank %>% filter(Family_ID %in% terminal_only) %>% arrange(desc(total)) %>% pull(Family_ID)
...
all_priority <- priority_info %>% pull(Family_ID) %>% unique()
...
total_binned <- binnedAverage(genome_windows, total_cov, "fraction") %>% as_tibble() %>% mutate(chr = sub("SUPER_", "Chr ", seqnames))
...
for (fam_id in priority_families) {
  ...
  binned <- binnedAverage(genome_windows, fam_cov, "fraction") %>% as_tibble() %>% mutate(Family_ID = fam_id)
...
pA <- ggplot() +
  ...
  geom_bar(data = tr_plot_data, aes(x = start, y = fraction, fill = Family_ID), stat = "identity", position = "stack", width = WINDOW_SIZE) +
  ...
  labs(y = "TR Fraction", fill = "Major TR Families") +
  theme_panel + theme(legend.position = "top", legend.text = element_text(size = 7), legend.key.size = unit(0.3, "cm")) +
...
pC <- ggplot() +
  geom_rect(data = grs_regions_plot, aes(xmin = Start, xmax = End, ymin = 0, ymax = 1), fill = "wheat", alpha = 0.2) +
  geom_rect(data = rna_all, aes(xmin = Start, xmax = End, ymin = 0.05, ymax = 0.95, fill = type, color = type), linewidth = 0.5) +
  geom_vline(data = internal_grs_boundaries, aes(xintercept = position), linetype = "dotted", color = "black", alpha = 0.5) +
  geom_point(data = internal_grs_boundaries, aes(x = position, y = 1.05), shape = 25, fill = "black", size = 1, color = "black") +
  geom_rect(data = chr_rects, aes(xmin = 0, xmax = size, ymin = 0, ymax = 1), fill = NA, color = "black", linewidth = 0.5) +
  facet_grid(chr ~ .) +
  ...
  labs(y = "RNAs", x = "Genomic Position", fill = "RNA Type") +
  guides(color = "none") +
  theme_panel + theme(legend.position = "bottom", axis.title.x = element_text(), axis.text.x = element_text(), axis.ticks.x = element_line())
